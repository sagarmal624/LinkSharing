...
To use TableTools, first download DataTables ( http://datatables.net/download ) and place the unzipped TableTools package into a `extensions` directory in the DataTables package (in DataTables 1.9- use the `extras` directory). This will allow the pages in the UserProfileAndLockScreen to operate correctly. To see the UserProfileAndLockScreen running, open the `UserProfileAndLockScreen` directory in your web-browser.
